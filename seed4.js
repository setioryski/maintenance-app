// seed4.js
// This script initializes 100 assets for the ELEKTRIKAL division,
...
/**
 * Seeds the database with 100 assets for the ELEKTRIKAL division,
 * distributing them across all floors and zones.
 */
async function seed100ElektrikalAssets() {
    console.log('Starting to seed 100 assets for the ELEKTRIKAL division...');

    try {
        // --- Get references for Division and Category ---
        const elektrikalDivision = await Division.findOne({ name: 'ELEKTRIKAL' });
        const pillarCategory = await AssetCategory.findOne({ name: 'Pillar' });

        if (!elektrikalDivision || !pillarCategory) {
            console.error('Could not find necessary references (ELEKTRIKAL Division or Pillar Category). Please run the initial seed script first.');
            return;
        }
        ...
        for (let i = 0; i < 100; i++) {
            ...
            const assetName = `PANEL-E-${String(assetCounter).padStart(3, '0')}`;
            ...
            assetsToCreate.push({
                name: assetName,
                description: `Panel elektrikal #${assetCounter}`,
                location: `Area ${assetCounter}`,
                category: pillarCategory._id,
                floor: floor._id,
                zone: zone._id,
                division: elektrikalDivision._id
            });
            assetCounter++;
        }
        ...
        if (assetsToCreate.length > 0) {
            ...
            console.log(`✅ Successfully created ${assetsToCreate.length} new assets for the ELEKTRIKAL division.`);
        } else {
